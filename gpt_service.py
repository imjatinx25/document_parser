...
async def analyze_table_structure(tables: Dict[str, List[List[str]]]) -> Dict:
    """
    Analyze the structure of extracted tables to understand headers and format.
    Logs raw GPT response on each retry.
    """
    try:
        if not tables:
            raise Exception("No tables found in the PDF")
        ...
        while retry_count < max_retries:
            try:
                ...
                response = await openai_client.chat.completions.create(
                    model='gpt-4.1-nano',
                    messages=[
                        {"role": "system", "content": "You are a financial data analysis expert. Analyze table structure and extract headers and example transactions."},
                        {"role": "user", "content": analysis_prompt}
                    ],
                    response_format={"type": "json_object"},
                    temperature=0.0
                ...
                print(f"\n[DEBUG] Raw GPT Response Attempt {retry_count + 1}:\n{raw_content}\n")
                ...
                print(f"   - Warning: JSONDecodeError: {str(e)}")
                retry_count += 1
            except ValueError as e:
                print(f"   - Warning: Validation Error: {str(e)}")
                retry_count += 1
            except Exception as e:
                print(f"   - Warning: General Error: {str(e)}")
                retry_count += 1
...
async def extract_transactions(tables, context, chunk_size=None):
    transactions = []

    average_table_length = int(sum(len(rows) for rows in tables.values()) / len(tables)) if tables else 0
    chunk_size = chunk_size or calculate_dynamic_chunk_size(len(tables), average_table_length)
    table_keys = list(tables.keys())

    async def process_chunk(chunk):
        prompt = get_extraction_prompt(chunk, context)
        response = await run_gpt(prompt)
        return response.get('transactions', [])

    tasks = []
    for i in range(0, len(table_keys), chunk_size):
        chunk_keys = table_keys[i:i + chunk_size]
        chunk_data = {k: tables[k] for k in chunk_keys}
        tasks.append(process_chunk(chunk_data))
...
async def categorize_transactions(transactions, chunk_size=None):
    categorized = []
    chunk_size = chunk_size or calculate_dynamic_chunk_size(len(transactions))

    async def process_chunk(chunk):
        prompt = get_categorization_prompt(chunk)
        response = await run_gpt(prompt)
        return response.get('transactions', [])
